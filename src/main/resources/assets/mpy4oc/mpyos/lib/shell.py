...
class Shell:
    ...
    def loop(self):
        while self.running:
            # If the screen went away (monitor broken, or the SFTP card's virtual
            # screen just came up), rebind to whatever is available now. Passive:
            # costs one getScreen() when nothing changed.
            try:
                self.tty.ensure_screen()
            except Exception:
                pass
