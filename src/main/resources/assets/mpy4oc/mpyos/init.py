...
def _bind_screen(gpu_addr):
    """Bind the gpu to a screen so it can draw, and return a Terminal.

    A missing screen is NOT fatal: we still return a Terminal bound to the gpu, so
    the system boots headless and picks up a screen as soon as one appears (a
    monitor is placed). The shell rebinds
    passively on each prompt -- see Terminal.ensure_screen().
    """
    gpu = component.proxy(gpu_addr)
    screen_addr = _first("screen")
    if screen_addr is not None:
        try:
...
def main():
    ...
    # No screen right now is fine: the terminal will bind to one when it shows up
    # (monitor placed). Beep so a headless boot
    # is still noticeable, then carry on into the shell.
    if tty.bound_screen() is None:
        try:
            computer.beep(880, 0.3)
        except Exception:
            pass
